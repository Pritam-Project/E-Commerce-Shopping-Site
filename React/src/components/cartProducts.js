...
function CartProduct() {
    ...
    useEffect(() => {
      axios.get('/product/carts/mycart/'+useremail)
        .then(response => {
          console.log(response.data);
          // mm = response.data.productprice;
         for(let i=0 ; i< response.data.length ; i++)
         {
            myarr.push(response.data[i].productprice);
         }
      ...
      return prodlist.map((currentrow, index) => {
        return (
          <tr key={index}>
            ...
            <td>x {currentrow.productquantity}</td>
            <td>{currentrow.productprice}</td>
            <td>{currentrow.productsize}</td>
            <td>{currentrow.productPicture}</td>
            <td>{currentrow.updatedAt}</td>
            <td><button type="submit" className="btn btn-primary" onClick={() => RemoveFromCart(currentrow.productid) }>Remove</button></td>
            <td><button type="submit" className="btn btn-primary" onClick={() => DeleteFromCart(currentrow._id,index) }>Delete</button></td>
            <td><button type="submit" className="btn btn-secondary" onClick={() => MoveToOrderList(currentrow.cartproductid) }>BUY NOW</button></td>
          </tr>
    ...
    function DeleteFromCart(currentrowid,index) {
      ...
      axios.delete('/product/cart/removeitem/'+currentrowid).then(
        setEList(tempprolist),
        axios.get('/product/carts/mycart')
        .then(response => {
    ...
    function RemoveFromCart(currentrowid){
      ...
      axios.get('/product/carts/removeone/'+currentrowid).then(() =>{
        axios.get('/product/carts/mycart/'+ useremail)
        .then(response => {
